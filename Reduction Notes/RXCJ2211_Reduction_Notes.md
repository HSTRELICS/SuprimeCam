...
# Cluster Specific Summary Notes
* For the 2009 exposures but should be SDFRED1 for the 2000-2007 exposures.
* Used i-band as the deep band, because deeper RC from 2000 was missing one of the chips.
* Had to run through i-band using 2MASS for astrometric reference but then run back through using that stacked image for astrometry, which fixed some of the frames.
* There is V-band data from 2003 (36 minutes) and 2009 (28 minutes), both seem to have better than 1" seeing so I think we could stack them together.
...
This portion of the pipeline uses SDFRED and the procedure documented in its [manual](http://www.naoj.org/Observing/Instruments/SCam/sdfred/v2.0/sdfred2_2p1ae.pdf).
...
Both 2MASS and SDSS produced poor stacks (despite no frame failures) so I ran
through the pipeline, and then used the Subaru catalog as the astrometry
reference and everything looks good.
